...
const useGetRLateral = ({ record, pos, setPos, posMax, setPosMax }:
	{
		record: boolean,
		pos: number,
	...
	const runDummy = interval(async () => {
		...
		//console.log(`useGetRLateral fetchData: ${randomDecimal}`)
	...
	useEffect(() => {
		...
		if (!record) {
			runDummy.stop();
			dispatch(updateROM({ key: "r_lateral", value: randomDecimal }));
