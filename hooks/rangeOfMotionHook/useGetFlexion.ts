...
const useGetFlexion = ({ record, pos, setPos, posMax, setPosMax }:
	{
		record: boolean,
		pos: number,
	...
	const runDummy = interval(async () => {
		...
		//console.log(`useGetFlexion fetchData: ${randomDecimal}`)
	...
	useEffect(() => {
		...
		if (!record) {
			runDummy.stop();
			dispatch(updateROM({ key: "flexion", value: randomDecimal }));
