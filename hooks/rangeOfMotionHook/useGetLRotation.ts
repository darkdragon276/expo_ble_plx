...
const useGetLRotation = ({ record, pos, setPos, posMax, setPosMax }:
	{
		record: boolean,
		pos: number,
	...
	const runDummy = interval(async () => {
		...
		//console.log(`useGetLRotation fetchData: ${randomDecimal}`)
	...
	useEffect(() => {
		...
		if (!record) {
			runDummy.stop();
			dispatch(updateROM({ key: "l_rotation", value: randomDecimal }));
...
export default useGetLRotation
