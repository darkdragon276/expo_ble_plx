...
const useGetLLateral = ({ record, pos, setPos, posMax, setPosMax }:
	{
		record: boolean,
		pos: number,
	...
	const runDummy = interval(async () => {
		...
		//console.log(`useGetLLateral fetchData: ${randomDecimal}`)
	...
	useEffect(() => {
		...
		if (!record) {
			runDummy.stop();
			dispatch(updateROM({ key: "l_lateral", value: randomDecimal }));
